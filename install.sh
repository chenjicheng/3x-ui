...
configure_sso() {
    ...
    # Allow operator to pick which IdToken claim becomes the panel display
    # name. Default is `email`, which is what Pocket-ID emits. It's purely
    # cosmetic / logging — authorization is enforced via the allow-list below.
    local username_claim=""
    _read_or_exit username_claim "Claim to show as panel username (display only) [email]: " || return 1
